...
export default function initStripe() {
  if (!process.env.STRIPE_SECRET_KEY) {
    console.warn("Stripe API key not configured. Payment functionality is disabled.");
    return null;
  }
...
export const stripe = new Proxy({}, {
  get(target, prop) {
    ...
    if (!instance) {
      console.warn(`Stripe method '${String(prop)}' called but Stripe is not configured`);
