  ...
  async getNordicDestinations() {
    ...
    return {
      norway: nordicDestinations.filter(d => 
        d.destinationName?.toLowerCase().includes('norway') || 
        ['oslo', 'bergen', 'trondheim', 'tromsø'].some(city => 
          d.destinationName?.toLowerCase().includes(city)
        )
      ),
      sweden: nordicDestinations.filter(d => 
        d.destinationName?.toLowerCase().includes('sweden') || 
        ['stockholm', 'gothenburg', 'malmo'].some(city => 
          d.destinationName?.toLowerCase().includes(city)
        )
      ),
      iceland: nordicDestinations.filter(d => 
        d.destinationName?.toLowerCase().includes('iceland') || 
        ['reykjavik', 'akureyri'].some(city => 
          d.destinationName?.toLowerCase().includes(city)
        )
      ),
      denmark: nordicDestinations.filter(d => 
        d.destinationName?.toLowerCase().includes('denmark') || 
        ['copenhagen', 'aarhus'].some(city => 
          d.destinationName?.toLowerCase().includes(city)
        )
      ),
      finland: nordicDestinations.filter(d => 
        d.destinationName?.toLowerCase().includes('finland') || 
    ...
    for (const [country, destinations] of Object.entries(nordicDestinations)) {
      ...
      try {
        ...
        const productsWithCountry = products.map(product => ({
          ...product,
          _country: country,
          _destinationName: mainDestination.destinationName
        }));

        allProducts.push(...productsWithCountry);
      } catch (error) {
