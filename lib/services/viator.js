...
export class ViatorService {
  ...
  async getProductPricing(productCode) {
    ...
    const extractNumericPrice = (priceData) => {
      if (priceData === null || priceData === undefined) return 0;
      if (typeof priceData === 'number') return priceData;
      if (typeof priceData === 'string') return parseFloat(priceData) || 0;
      if (typeof priceData === 'object') {
        return Number(priceData.amount ?? priceData.value ?? priceData.price ?? priceData.recommendedRetailPrice ?? 0);
      }
      return 0;
    };
    
    try {
      // Request with USD currency
      const response = await fetch(`${VIATOR_API_BASE_URL}/availability/schedules/${productCode}?currency=USD`, {
        method: 'GET',
        headers: this.baseHeaders,
      });
      ...
      // Extract fromPrice from availability schedules response (as numeric)
      if (data.summary?.fromPrice) {
        const price = extractNumericPrice(data.summary.fromPrice);
                    ...
                    for (const entry of record.timedEntries) {
                      if (entry.pricingDetails) {
                        for (const detail of entry.pricingDetails) {
                          const price = extractNumericPrice(detail.price || detail.retailPrice || detail.merchantNetPrice);
