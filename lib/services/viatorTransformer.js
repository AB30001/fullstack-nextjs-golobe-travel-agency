...
export function transformViatorToExperience(viatorProduct) {
  ...
  let priceValue = 0; // Default - will check if valid at end
  
  // Check all possible price locations in order of priority
  // 1. Direct fromPrice (from availability/schedules)
  if (viatorProduct.fromPrice && typeof viatorProduct.fromPrice === 'number' && viatorProduct.fromPrice > 0) {
    priceValue = viatorProduct.fromPrice;
  }
  // 2. pricingInfo.summary.fromPrice (product details)
  else if (viatorProduct.pricingInfo?.summary?.fromPrice) {
    const fp = viatorProduct.pricingInfo.summary.fromPrice;
    priceValue = typeof fp === 'object' ? (fp.amount || fp.value || 0) : fp;
  }
  // 3. pricingInfo.fromPrice (older format)
  else if (viatorProduct.pricingInfo?.fromPrice) {
    const fp = viatorProduct.pricingInfo.fromPrice;
    priceValue = typeof fp === 'object' ? (fp.amount || fp.value || 0) : fp;
  }
  // 4. pricing.summary.fromPrice (search results)
  else if (viatorProduct.pricing?.summary?.fromPrice) {
    const fp = viatorProduct.pricing.summary.fromPrice;
    priceValue = typeof fp === 'object' ? (fp.amount || fp.value || 0) : fp;
  }
  // 5. pricing.fromPrice
  else if (viatorProduct.pricing?.fromPrice) {
    const fp = viatorProduct.pricing.fromPrice;
    priceValue = typeof fp === 'object' ? (fp.amount || fp.value || 0) : fp;
  }
  // 6. price.fromPrice
  else if (viatorProduct.price?.fromPrice) {
    const fp = viatorProduct.price.fromPrice;
    priceValue = typeof fp === 'object' ? (fp.amount || fp.value || 0) : fp;
  }
  // 7. _existingPrice (passed from existing tour for refresh)
  else if (viatorProduct._existingPrice && viatorProduct._existingPrice > 0) {
    priceValue = viatorProduct._existingPrice;
  }
  
  // Log for debugging if price is still 0
  if (priceValue === 0 || priceValue === 100) {
    console.log(`Price extraction for ${viatorProduct.productCode}: checking fields...`, {
      directFromPrice: viatorProduct.fromPrice,
      pricingInfoSummary: viatorProduct.pricingInfo?.summary?.fromPrice,
      pricingInfoFromPrice: viatorProduct.pricingInfo?.fromPrice,
      pricingSummary: viatorProduct.pricing?.summary?.fromPrice,
