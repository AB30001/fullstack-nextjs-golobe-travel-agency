    ...
    viatorProduct.images.forEach(img => {
      if (img.variants && img.variants.length > 0) {
        ...
        if (largestVariant.url) {
          images.push(largestVariant.url);
        }
      }
    });
  }
  
  if (images.length === 0 && viatorProduct.productImages) {
...
export function transformViatorToExperience(viatorProduct) {
  ...
  const price = viatorProduct.pricing?.summary?.fromPrice || viatorProduct.price?.fromPrice || 100;
  ...
  const affiliateLink = viatorProduct.productUrl || 
                       viatorProduct.productUrlName || 
                       (productCode ? `https://www.viator.com/tours/productCode/${productCode}` : 'https://www.viator.com');
  
  const highlights = viatorProduct.highlights || [];
  const included = viatorProduct.inclusions || [];
  const notIncluded = viatorProduct.exclusions || [];
